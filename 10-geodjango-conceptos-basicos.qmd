...
El [Django REST Framework (DRF)](https://www.django-rest-framework.org/) es una biblioteca poderosa y flexible para crear interfaces de programación de aplicaciones (API) para la Web en Django. Es ampliamente utilizado en el desarrollo de servicios *backend* que necesitan proporcionar datos al *frontend* mediante solicitudes HTTP, permitiendo la creación de arquitecturas basadas en [microservicios](https://es.wikipedia.org/wiki/Arquitectura_de_microservicios) y aplicaciones modernas y escalables.

26. Para utilizar el DRS, es necesario instalar un par de módulos adicionales en nuestro ambiente Conda:
...
27. Edite el archivo `mapa_leaflet_postgis/settings.py` y agregue las aplicaciones:
...
29. Utilice el serializador que acaba de crear en dos vistas. Edite el archivo `mapa_backend/views.py` y agregue el siguiente contenido (antes comente el código de las vistas que creó en los pasos anteriores):
...
Seguidamente, se muestra un ejemplo de las funciones espaciales de GeoDjango. Se define una vista para mostrar las tres ciudades más cercanas a una observación. Debe agregarse contenido a los siguientes archivos:

`mapa_backend/serializers.py`:

```python
class CiudadSerializador(GeoFeatureModelSerializer):
...
class CiudadLista(generics.ListAPIView):

    serializer_class = CiudadSerializador
    name = 'ciudad-lista'

    def get_queryset(self):
        observacionID = self.request.query_params.get('observacionid')

        if observacionID is None:
            raise Http404
        
        geom_observacion_seleccionada = get_object_or_404(Observacion, pk=observacionID).point_geom
        ciudades_cercanas = Ciudad.objects.annotate(distance=Distance('point_geom', geom_observacion_seleccionada)).order_by('distance')[:3]

        return ciudades_cercanas
```

`mapa_backend/urls.py`:

```python
urlpatterns = [
    path("observaciones/", views.ObservacionLista.as_view(), name=views.ObservacionLista.name),
    path("observaciones/<int:pk>/", views.ObservacionDetalle.as_view(), name=views.ObservacionDetalle.name),
    path("ciudades/", views.CiudadLista.as_view(), name=views.CiudadLista.name)
    # path("observaciones/", views.observaciones, name="observaciones"),
    # path("observacion_por_llave/<int:llave>", views.observacion_por_llave, name="observacion_por_llave"),
    # path("ciudades/", views.ciudades, name="ciudades"),
]
...
Las tres ciudades más cercanas a una observación pueden consultarse, por ejemplo, con:

[http://127.0.0.1:8000/api/v1/ciudades/?observacionid=2](http://127.0.0.1:8000/api/v1/ciudades/?observacionid=2)
